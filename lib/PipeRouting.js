...
export class PipeRouting {
    ...
    async RapidPipeRoutingStart(equipmentsList, startPoint, endPoint, visualization = false, dynamiCoeff = 1) {
        ...
        const newStartPoint = new GridPos(
            Math.floor((startPoint.x - originPoint.x) / this.DynamicCoeff),
            Math.floor((startPoint.y - originPoint.y) / this.DynamicCoeff),
            Math.floor((startPoint.z - originPoint.z) / this.DynamicCoeff)
        );
        const newEndPoint = new GridPos(
            Math.floor((endPoint.x - originPoint.x) / this.DynamicCoeff),
            Math.floor((endPoint.y - originPoint.y) / this.DynamicCoeff),
            Math.floor((endPoint.z - originPoint.z) / this.DynamicCoeff)
        );
        ...
        const resultPathList = AStarFinder.findPath(aStarParam);



        // Expand/modify result path and reset MovableMatrix
        let modifiedResultPathList = resultPathList.map(pos => {
            return new GridPos(
        ...
        dynamicResultPathList.push(endPoint);
        ...
        console.log(pipeLength, sumNumofBend, avgFeasibilityIndex, avgSpaceAvailability);
        

        return dynamicResultPathList;
    }
}
...
function CreateLimitedArea(startPoint, endPoint, equipmentsList, dynamicCoeff) {
    const margin = 10;
    ...
    const equipments =  Object.entries(equipmentsList);
    ...
    for (let i = 0; i < equipments.length; i++) {
        ...
        let newEquipment = equipments[i][1]
        dynamicEquipments.push(newEquipment);

        let lastEquipment = dynamicEquipments[dynamicEquipments.length - 1];
        lastEquipment.MinX = (equipments[i][1].min_x - originPoint.x) / dynamicCoeff;
        lastEquipment.MaxX = (equipments[i][1].max_x - originPoint.x) / dynamicCoeff;
        lastEquipment.MinY = (equipments[i][1].min_y - originPoint.y) / dynamicCoeff;
        lastEquipment.MaxY = (equipments[i][1].max_y - originPoint.y) / dynamicCoeff;
        lastEquipment.MinZ = (equipments[i][1].min_z - originPoint.z) / dynamicCoeff;
        lastEquipment.MaxZ = (equipments[i][1].max_z - originPoint.z) / dynamicCoeff;
